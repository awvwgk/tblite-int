...
contains
   ...
   subroutine main(config, error)
      ...
      character(len=:), allocatable :: method
      real(wp) :: cutoff
      real(wp), allocatable :: selfenergy(:), lattr(:, :)
      ...
      call get_selfenergy(calc%h0, mol%id, calc%bas%ish_at, calc%bas%nsh_id, cn=config%cn, &
      ...
      do while (iarg < narg)
         ...
         case ("--cn")
            ...
            if (.not. allocated(arg)) then
               call fatal_error(error, "Missing argument for coordination number")
               exit
            end if
            allocate (config%cn(2))
            call get_argument_as_realv(arg, config%cn, error)
            if (allocated(error)) exit

         case ("--hamiltonian")
            config%hamiltonian = .true.
   ...
   subroutine help(io)
      ...
      character(len=*), parameter :: help_text = &
                                     ...
                                     "      --cn <real>,<real>  Coordination number of the two atomic species"//nl// &
   ...
   subroutine write_matrix(matrix, name, unit, step)
      ...
      do i = 1, d2, istep
         ...
         do k = i, l
            write (iunit, '(9x,i7)', advance='no') k
         end do
         write (iunit, '(a)')
         do j = 1, d1
